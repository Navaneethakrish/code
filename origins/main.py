...
class Reports(openerpweb.Controller):
    ...
    def index(self, req, action, token):
        ...
        if file_name == 'Purchase Order':
          ...
          # file_name =  purchase_order_obj.partner_id.name +''+ purchase_order_obj.name 
        ...
        # This is to get the reference name of the proforma invoice
        sale_order_name = context["active_id"]
        sale_uid = context["uid"]        
        if file_name == 'Proforma Invoice':
            dbname = req.session._db
            db = sql_db.db_connect(dbname) 
            cr = db.cursor()            
            pool =  pooler.get_pool(cr.dbname)
            ...
            so_name=sale_order_obj.name
            sql_db.close_db(dbname)  
            file_name = dbname+'_PI-'+ so_name[3:] 
